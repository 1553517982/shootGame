...
class ObjectPool {
    ...
    /**animation factory */
    /**
     * 根据groupName获取帧动画Factory
     */
    public static getMovieClipFactory(groupName: string): egret.MovieClipDataFactory {
        if (ObjectPool._factory[groupName]) {
            return ObjectPool._factory[groupName];
        } else {
            let data = RES.getRes(groupName + '_json');
            let texture = RES.getRes(groupName + '_png');
            let mcFactory = new egret.MovieClipDataFactory(data, texture);
            ObjectPool._factory[groupName] = mcFactory;
            return mcFactory;
        }
    }
}