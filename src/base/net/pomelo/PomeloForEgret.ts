        ...
        */
        ...
        private onConnect(e: egret.Event): void {
            ...
            if (this._connectFlag && this._needReconnect) {
                App.EventSystem.dispatch(GameEvent.E_SOCKET_RECONNECT);
            } else {
                App.EventSystem.dispatch(GameEvent.E_SOCKET_CONNECT);
            }
        ...
        private onClose(e: egret.Event): void {
            ...
            if (this._needReconnect) {
                App.EventSystem.dispatch(GameEvent.E_SOCKET_START_RECONNECT);
                this.reconnect();
            } else {
                App.EventSystem.dispatch(GameEvent.E_SOCKET_CLOSE);
            }
        }
        ...
        private onIOError(e: egret.Event): void {
            ...
            if (this._needReconnect) {
                ...
                App.EventSystem.dispatch(GameEvent.E_SOCKET_NOCONNECT);
            }
            this._isConnecting = false;
        }
        ...
        public connect(): void {
            ...
            this.addEvents();
            if (this._host.match(/\d+\.\d+\.\d+\.\d+/)) {
                this.socket.connect(this._host, this._port);
            } else {
                this.socket.connectByUrl("wss://" + this._host + ":" + this._port);
            }
        }
    ...
    class Message implements IMessage {
        ...
        public decode(buffer: egret.ByteArray): any {
            ...
            if (type === Message.TYPE_REQUEST || type === Message.TYPE_NOTIFY || type === Message.TYPE_PUSH) {
                ...
                route = Pomelo.routeMap[id];
            }

            if (!id && !(typeof (route) == "string")) {
                route = Routedic.getName(route);
            }
            console.warn("id:", id, "rout:", route)
